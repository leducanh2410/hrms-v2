...
import { LuongdialogComponent } from '../../qtrluong/luongdialog/luongdialog.component';
...
import { ThamsoluongformComponent } from '../../qtrluong/luongdialog/thamsoluongform/thamsoluongform.component';
...
@Component({
  selector: 'app-luongdieuchinhdialog',
  templateUrl: './luongdieuchinhdialog.component.html',
  styleUrls: ['./luongdieuchinhdialog.component.scss'],
  imports:[
...
export class LuongdieuchinhdialogComponent implements OnInit {
  ...
  constructor(
    ...
    public matDialogRef: MatDialogRef<LuongdieuchinhdialogComponent>,
  ...
  onCheckKcanhbao(): void {
    if (this.data.canhbaonl == false) { this.data.lydokocanhbao = ''; }
  ...
  onChangeNgayHieuLuc(): void {
    ...
    if (!this.data.ckeckLuongKhoan) {
      this.http
        .get(QuatrinhLuongURL.getThoiGianNangLuongLanSau(this.data.ngayhieuluc, this.data.bacluongId, this.data.donviId))
        .pipe(takeUntil(this._unsubscribeAll))
        .subscribe((res: any) => {
          if (!res || !res.state) return;
  ...
  openSoQDbyTxt(event: any): void {
    this.http
      .post(DanhMucURL.postNsQdndungBySoQd(), { soQd: event.target.value, donviId: this.data.donviId })
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe((res: any) => {
        if (res.state) {
          ...
          this.nguoikyQdBean = this.listNguoiKy.find(element => element.tenkhaisinh === res.data.nguoiky)
          if (this.nguoikyQdBean == null) {
            this.nguoikyQdBean = {
              id: null,
    ...
    dialogRef.afterClosed()
      .subscribe((result) => {
        if (result) {
          ...
          this.nguoikyQdBean = this.listNguoiKy.find(element => element.tenkhaisinh === result.nguoiky)
          if (this.nguoikyQdBean == null) {
            this.nguoikyQdBean = {
              id: null,
  ...
  async onSave() {
    if (this.insertFile && this.insertFile.length != 0) this.data.fileQdinh = this.insertFile[this.insertFile.length - 1]
    if (this.fileContent != null) {
      this.data.fileQdinh.fileContent = this.fileContent;
    }

    if (this.insertFilePhuluc && this.insertFilePhuluc.length != 0) this.data.fileLuongDinhkem = this.insertFilePhuluc[this.insertFilePhuluc.length - 1]
    ...
    this.http
      ...
      .subscribe((res: any) => {
        ...
        if (this.data != null && this.data.nsLuongId != null) {
          ...
          this.http
            .post(QuatrinhLuongURL.updateNsLuongDieuchinh(), this.data)
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe((res: any) => {
              if (!res || !res.state) {
                this.messageService.showErrorMessage(
          ...
          this.http
            .post(QuatrinhLuongURL.insertNsLuongDieuchinh(), this.data)
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe((res: any) => {
              if (!res || !res.state) {
                this.messageService.showErrorMessage(
  ...
  onSelectNguoiKy(event) {
    let nguoiky = event;
    console.log(nguoiky);
    this.chucvuQdBean = this.listChucvuQd.find(element => element.name === nguoiky.chucvuKy)
    if (this.chucvuQdBean == null) {
      this.chucvuQdBean = {
        id: null,
  ...
  async myUploaderFilePhuluc(event, _fileFormPhuluc) {
    ...
    const files = this.uploadedFilesPhuluc[this.uploadedFilesPhuluc.length - 1].currentFiles;
    if (this.insertFilePhuluc == null)
      this.insertFilePhuluc = []
    files.forEach(async (file) => {
      await this.blobToBase64(file)
        .then((base64data) => {
