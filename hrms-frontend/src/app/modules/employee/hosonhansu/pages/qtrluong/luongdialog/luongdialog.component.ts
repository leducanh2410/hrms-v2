...
export class LuongdialogComponent implements OnInit, OnDestroy {
  ...
  onChangeMoctinhLuong() {
    if (!this.data.ckeckLuongKhoan) {
      if (this.data.thoiGianCanCuNangL) {
        this.http
          .get(QuatrinhLuongURL.getThoiGianNangLuongLanSau(this.data.thoiGianCanCuNangL, this.data.bacluongId, this.data.donviId))
          .pipe(takeUntil(this._unsubscribeAll))
          .subscribe((res: any) => {
            if (!res || !res.state) return;
            this.data.moctinhnl = res.data;
  ...
  onKeyDownBacluong(event: KeyboardEvent) {
    if (event.key === 'Tab') {
      this.http
        .get(QuatrinhLuongURL.getLBacluongForSearch(this.data.mabangluong, this.data.mangachluong, this.data.bacluong))
        .pipe(takeUntil(this._unsubscribeAll))
        .subscribe((res: any) => {
          if (!res || !res.state) {
            this.data.heSo = null;
